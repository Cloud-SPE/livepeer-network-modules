...
func (m *Mock) GetSpendAuthorization(_ context.Context, payer []byte, authorizationID string, wholesaleAccountID string) (*SpendAuthorizationStatus, error) {
	...
	return &SpendAuthorizationStatus{SettlementDomainID: MockSettlementDomainID, Payee: append([]byte(nil), auth.payload.GetPayee()...), WholesaleAccountID: auth.payload.GetWholesaleAccountId(), State: auth.state, Reserved: new(big.Int).Set(auth.reserved), Billed: new(big.Int).Set(auth.billed), Released: new(big.Int).Set(auth.released)}, nil
}
