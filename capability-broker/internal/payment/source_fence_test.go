...
type sourceRPC struct{ *revenueRPC }
...
func (r *sourceRPC) CloseUnexecutedAuthorization(context.Context, *pb.CloseUnexecutedAuthorizationRequest, ...grpc.CallOption) (*pb.CloseUnexecutedAuthorizationResponse, error) {
	return &pb.CloseUnexecutedAuthorizationResponse{SettlementDomainId: r.reportDomain, Fenced: true}, nil
}
func TestSourceControlAndRecoveryStayBoundThroughMetrics(t *testing.T) {
	domain := "0x" + strings.Repeat("a", 64)
	rpc := &sourceRPC{&revenueRPC{domain: domain, reportDomain: domain}}
	...
	for _, wrong := range []bool{false, true} {
		...
		recoverErr := recovery.CloseUnexecutedAuthorization(ctx, make([]byte, 20), "orphan", "no binding")
