...
func TestPaymentAccountAuthorizationReservesAndSettlesActual(t *testing.T) {
	...
	mw := Payment(client, stubLookup, InterimDebitConfig{}, nil, func(in *paymentsv1.SettlementRecord) (string, error) {
		settlement = proto.Clone(in).(*paymentsv1.SettlementRecord)
		return "signed-settlement", nil
	}, nil, "https://broker.example/")
...
func TestPaymentAccountAuthorizationRejectsAlteredBodyBeforeAdmission(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	Payment(client, stubLookup, InterimDebitConfig{}, nil, nil, nil, "https://broker.example")(http.HandlerFunc(func(http.ResponseWriter, *http.Request) { t.Fatal("backend ran") })).ServeHTTP(rec, r)
