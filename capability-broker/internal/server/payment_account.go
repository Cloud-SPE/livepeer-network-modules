...
func (s *Server) handlePaymentAccountFund(w http.ResponseWriter, r *http.Request) {
	...
	accountID := r.Header.Get(livepeerheader.WholesaleAccountID)
	if !identity.ValidWholesaleAccountID(accountID) {
		http.Error(w, "valid Livepeer-Wholesale-Account-Id is required", http.StatusBadRequest)
		return
	}
	...
	result, err := ac.FundWholesaleAccount(r.Context(), paymentBytes, accountID)
	...
	if result.Account.WholesaleAccountID != accountID || result.FundingID != hex.EncodeToString(digest[:]) {
		http.Error(w, "funding receipt identity mismatch", http.StatusBadGateway)
		return
	}
	w.Header().Set("Cache-Control", "no-store")
	writeJSON(w, http.StatusOK, map[string]any{
		"wholesale_account_id": result.Account.WholesaleAccountID, "isolation_version": 1, "funding_id": result.FundingID, "payer": "0x" + hex.EncodeToString(result.Account.Payer), "payee": "0x" + hex.EncodeToString(result.Account.Payee),
		"credited_value_wei": decimalString(result.Credited), "available_value_wei": decimalString(result.Account.Available),
		"settlement_domain_id": result.Account.SettlementDomainID, "account_version": result.Account.Version, "replayed": result.Replayed,
	})
}
...
func paymentAccountHandler(client payment.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !identity.ValidWholesaleAccountID(in.WholesaleAccountID) {
			http.Error(w, "valid wholesale_account_id is required", http.StatusBadRequest)
			return
		}
		...
		if in.AuthorizationID != "" {
			status, err := ac.GetSpendAuthorization(r.Context(), payer, in.AuthorizationID, in.WholesaleAccountID)
			if err != nil {
				http.Error(w, "get spend authorization: "+err.Error(), http.StatusBadGateway)
				return
			}
			if status == nil || status.WholesaleAccountID != in.WholesaleAccountID {
				http.Error(w, "authorization account mismatch", http.StatusBadGateway)
				return
			}
			_ = json.NewEncoder(w).Encode(map[string]any{"payee": "0x" + hex.EncodeToString(status.Payee), "settlement_domain_id": status.SettlementDomainID, "wholesale_account_id": status.WholesaleAccountID, "isolation_version": 1, "payer": "0x" + hex.EncodeToString(payer), "authorization_id": in.AuthorizationID, "state": authorizationStateJSON(status.State), "reserved_value_wei": decimalString(status.Reserved), "billed_value_wei": decimalString(status.Billed), "released_value_wei": decimalString(status.Released), "actual_units": status.ActualUnits, "settlement_seq": status.SettlementSeq, "observed_at": status.ObservedAt})
			return
		}
		account, err := ac.GetWholesaleAccount(r.Context(), payer, in.WholesaleAccountID)
		if err != nil {
			http.Error(w, "get wholesale account: "+err.Error(), http.StatusBadGateway)
			return
		}
		if account == nil || account.WholesaleAccountID != in.WholesaleAccountID {
			http.Error(w, "account identity mismatch", http.StatusBadGateway)
			return
		}
		_ = json.NewEncoder(w).Encode(map[string]any{"wholesale_account_id": account.WholesaleAccountID, "isolation_version": 1, "payer": "0x" + hex.EncodeToString(account.Payer), "payee": "0x" + hex.EncodeToString(account.Payee), "settlement_domain_id": account.SettlementDomainID, "chain_id": account.ChainID, "denomination": account.Denomination, "credited_value_wei": decimalString(account.Credited), "reserved_value_wei": decimalString(account.Reserved), "debited_value_wei": decimalString(account.Debited), "available_value_wei": decimalString(account.Available), "version": account.Version, "observed_at": account.ObservedAt})
	}
}
