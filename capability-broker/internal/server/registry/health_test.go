...
func TestHealthHandler_EmbedsMetadataStatus(t *testing.T) {
	...
	HealthHandler(mgr, meta, nil).ServeHTTP(rec, req)
...
func TestHealthHandler_EmbedsPoolSnapshotStatusWhenConfigured(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/registry/health", nil)
	rec := httptest.NewRecorder()
	HealthHandler(mgr, stubMetadataStatusSource{}, stubPoolStatusSource{
		status: map[string]poolsnapshot.Status{
...
func TestHealthHandler_OmitsPoolStatusWhenSnapshotUnconfigured(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/registry/health", nil)
	rec := httptest.NewRecorder()
	HealthHandler(mgr, stubMetadataStatusSource{}, stubPoolStatusSource{}).ServeHTTP(rec, req)
