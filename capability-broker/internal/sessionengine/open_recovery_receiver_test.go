	...
	for _, scenario := range []string{"unaccepted", "accepted", "accepted_usage", "lost_settlement", "local_write_failed"} {
		t.Run(scenario, func(t *testing.T) {
			...
			if scenario == "accepted_usage" {
				want = 7
				if _, err := client.AdvanceAuthorization(context.Background(), payment.AdvanceAuthorizationRequest{Payer: auth.Payload.Payer, AuthorizationID: "initial", CumulativeUnits: want, AdvanceSeq: 4, TargetReserved: big.NewInt(400)}); err != nil {
					t.Fatal(err)
				}
			}
			...
			if scenario == "unaccepted" {
				...
				if err != nil || first.GetSettlementSeq() == 0 || first.GetDebitedUnits() != want {
					t.Fatalf("invalid evidence: %v %v", first, err)
				}
				account, err := client.GetWholesaleAccount(context.Background(), auth.Payload.Payer)
