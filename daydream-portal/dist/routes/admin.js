...
// not duplicated here.
import { z } from "zod";
import { getWaitlistById, listWaitlist, setWaitlistDecision, } from "../repo/waitlist.js";
...
export function registerDaydreamAdminRoutes(app, deps) {
    const requireAdmin = adminAuthPreHandler(deps.adminAuthResolver);
    app.get("/admin/waitlist", { preHandler: requireAdmin }, async (req, reply) => {
    ...
    app.post("/admin/waitlist/:id/approve", { preHandler: requireAdmin }, async (req, reply) => {
        ...
        if (!updated) {
            ...
            req.log.warn({ id: row.id, customerId: customer.id }, "waitlist row state changed during approve");
        }
        await reply.send({
            waitlist_id: row.id,
    ...
    app.post("/admin/waitlist/:id/reject", { preHandler: requireAdmin }, async (req, reply) => {
    ...
    app.get("/admin/usage/summary", { preHandler: requireAdmin }, async (_req, reply) => {
