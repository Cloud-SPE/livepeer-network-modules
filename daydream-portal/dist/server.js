...
import { auth, createCustomerPortal, registerCustomerSelfServiceRoutes, } from "@livepeer-network-modules/customer-portal";
...
export async function buildServer(config) {
    ...
    const portal = createCustomerPortal({
        ...
        cacheTtlMs: 30_000,
        admin: config.adminTokens.length > 0 ? { tokens: config.adminTokens } : undefined,
    ...
    const adminResolver = portal.adminAuthResolver ??
