...
| 2.2, 2.3 (review) | `79b4db2` | Transcode team's review of `83802ba`: secrets minted before the reservation; every stage write checked and failing closed; an in-flight id with different content is `request_id_reuse`; the terminal write is confirmed before capacity release and outcome report, else the record stays pending with obligations met. Multi-card `ingest_port_taken` test. |
