...
```sh
cp infra/scenarios/archive/openai-gateway-manifest/.env.example infra/scenarios/archive/openai-gateway-manifest/.env
docker compose \
  -f infra/scenarios/archive/openai-gateway-manifest/docker-compose.yml \
  --env-file infra/scenarios/archive/openai-gateway-manifest/.env \
  up -d
```
...
docker logs $(docker compose -f infra/scenarios/archive/openai-gateway-manifest/docker-compose.yml ps -q service-registry-daemon) --tail 50
```
