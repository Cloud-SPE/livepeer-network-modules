...
```sh
cp infra/scenarios/archive/video-worker-node/.env.example infra/scenarios/archive/video-worker-node/.env
docker compose \
  -f infra/scenarios/archive/video-worker-node/docker-compose.yml \
  --env-file infra/scenarios/archive/video-worker-node/.env \
  up -d
```
