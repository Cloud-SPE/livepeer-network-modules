...
```sh
cp infra/scenarios/single-worker-node/.env.example infra/scenarios/single-worker-node/.env
docker compose \
  -f infra/scenarios/single-worker-node/docker-compose.yml \
  --env-file infra/scenarios/single-worker-node/.env \
  up -d
```
