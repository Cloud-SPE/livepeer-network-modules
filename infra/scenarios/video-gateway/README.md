...
```sh
cp infra/scenarios/video-gateway/.env.example infra/scenarios/video-gateway/.env
docker compose \
  -f infra/scenarios/video-gateway/docker-compose.yml \
  --env-file infra/scenarios/video-gateway/.env \
  up -d
```
