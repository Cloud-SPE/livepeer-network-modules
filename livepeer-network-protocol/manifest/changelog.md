...
| 1.0.0 | 2026-08-19 | **Breaking.** `interaction_mode` removed; replaced by required `protocol` (`<name>/v<major>`, e.g. `paid-job/v1`) plus a matching declared-axes object: `job.transports` for paid-job offerings, `session.{descriptor_schema, attachment, metering, refill, heartbeat, lease, tolerance_band_pct, runway_increment_units}` for paid-session offerings. Conditional rules pair each protocol with its axes object and forbid the other. Workload identity moves out of the protocol name into `session.descriptor_schema` (see `protocols/offering-axes.md`). Major bump: pre-v1 consumers cannot read v1 manifests and v1 consumers reject `interaction_mode`. |
