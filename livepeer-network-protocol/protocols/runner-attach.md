...
| `metering` | ✔ for `paid-session`; MUST be absent for `paid-job` | `runner-reported` \| `broker-observed` | `offering-axes.md` §3. `broker-observed` is valid only when the offer's session policy sets `attachment: inband-ws`; that check happens at match, not attach. | ✔ |
...
- **`metering: broker-observed` vs the offer's attachment mode**, GPU
  uniqueness across *other* enrollments, and every other rule that needs
