...
```bash
docker build -t tztcloud/openai-image-generation-runner:v1.2.0 .
```
...
docker run --rm \
  ...
  -e HF_TOKEN=hf_xxx \
  tztcloud/image-model-downloader:v1.2.0
```
...
| Image | Platforms |
|---|---|
| `openai-image-generation-runner:v1.2.0` | linux/amd64 only |
