...
func probeWholesale(ctx context.Context, cfg config, payer pb.PayerDaemonClient, payee pb.PayeeDaemonClient) error {
	...
	before, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
	...
	afterFirst, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
	...
	beforeReplay, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
	...
	final, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
...
func ensureWholesaleFloat(ctx context.Context, cfg config, payer pb.PayerDaemonClient, payee pb.PayeeDaemonClient, payerAddress []byte, target *big.Int, tag string) (*big.Int, error) {
	...
	if shortfall.Sign() > 0 {
		funded, err := payee.FundWholesaleAccount(ctx, &pb.FundWholesaleAccountRequest{WholesaleAccountId: cfg.wholesaleAccountID, SettlementDomainId: cfg.settlementDomainID, PaymentBytes: minted.GetPaymentBytes()})
		if err != nil {
			return nil, err
		}
...
func probeWholesaleConcurrency(ctx context.Context, cfg config, payer pb.PayerDaemonClient, payee pb.PayeeDaemonClient, payerAddress []byte, target *big.Int, body []byte) (*big.Int, error) {
	...
	baseline, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
	maxDebit := billFor(cfg.priceWei, cfg.perUnits, cfg.maxAuthUnits)
	...
	during, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
	...
	for _, i := range winners {
		if _, err := payee.SettleAuthorization(ctx, &pb.SettleAuthorizationRequest{WholesaleAccountId: cfg.wholesaleAccountID, SettlementDomainId: cfg.settlementDomainID, Payer: payerAddress, AuthorizationId: attempts[i].id, ActualUnits: 0, SettlementSeq: 1}); err != nil {
	...
	after, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
...
func probeWholesaleSession(ctx context.Context, cfg config, payer pb.PayerDaemonClient, payee pb.PayeeDaemonClient, payerAddress []byte, target *big.Int, delegated bool) (*big.Int, error) {
	...
	afterOpen, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
	...
	afterUsage, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
	...
	authState, err := payee.GetSpendAuthorization(ctx, &pb.GetSpendAuthorizationRequest{WholesaleAccountId: cfg.wholesaleAccountID, SettlementDomainId: cfg.settlementDomainID, Payer: payerAddress, AuthorizationId: gatewayID})
	if err != nil {
		return nil, err
	}
	...
	afterEnd, err := queryWholesaleAccount(cfg.brokerURL, payerAddress, cfg.settlementDomainID, cfg.wholesaleAccountID)
	if err != nil {
		return nil, err
	}
...
func createJobAuthorization(ctx context.Context, cfg config, payer pb.PayerDaemonClient, authorizationID, requestID string, body []byte, maxDebit *big.Int, callerPublicKey []byte) (*pb.CreateSpendAuthorizationResponse, error) {
	...
	now := time.Now().UTC()
	return payer.CreateSpendAuthorization(ctx, &pb.CreateSpendAuthorizationRequest{WholesaleAccountId: cfg.wholesaleAccountID, SettlementDomainId: cfg.settlementDomainID,
...
func mintAccountShortfallWithID(ctx context.Context, cfg config, payer pb.PayerDaemonClient, mintRequestID string, target, observed *big.Int) (*pb.CreatePaymentResponse, error) {
	return payer.CreatePayment(ctx, &pb.CreatePaymentRequest{
		...
		AccountFunding: &pb.AccountFundingIntent{WholesaleAccountId: cfg.wholesaleAccountID, SettlementDomainId: cfg.settlementDomainID,
...
func queryWholesaleAccount(brokerURL string, payer []byte, expectedDomain, expectedAccount string) (*wholesaleAccount, error) {
	body, _ := json.Marshal(map[string]string{"wholesale_account_id": expectedAccount, "payer_eth_address": "0x" + hex.EncodeToString(payer)})
	...
	if expectedAccount == "" || out.WholesaleAccountID != expectedAccount || out.IsolationVersion != 1 {
		return nil, fmt.Errorf("account isolation mismatch")
	}
