...
func TestCancelAuthorizationAdmissionRPC(t *testing.T) {
	...
	req := &pb.CancelAuthorizationAdmissionRequest{WholesaleAccountId: "test-account", SettlementDomainId: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Payer: bytes20(1), AuthorizationId: "revision", AuthorizationFingerprint: fp[:]}
	for i := 0; i < 2; i++ {
	...
	state, err := client.GetSpendAuthorization(context.Background(), &pb.GetSpendAuthorizationRequest{WholesaleAccountId: req.WholesaleAccountId, SettlementDomainId: req.SettlementDomainId, Payer: req.Payer, AuthorizationId: req.AuthorizationId})
	if err != nil || state.GetState() != pb.SpendAuthorizationState_SPEND_AUTHORIZATION_CANCELED_UNUSED || state.GetWholesaleAccountId() != req.WholesaleAccountId || state.GetActualUnits() != 0 || state.GetObservedAt() == "" {
		t.Fatalf("canceled state is not queryable: %+v %v", state, err)
	}
	req.WholesaleAccountId = ""
	if _, err := client.CancelAuthorizationAdmission(context.Background(), req); status.Code(err) != codes.InvalidArgument {
		t.Fatalf("missing account: %v", err)
	}
	req.WholesaleAccountId = "test-account"
