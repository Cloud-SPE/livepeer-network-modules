...
func TestDigestAndVerifyRejectMalformedAuthorizations(t *testing.T) {
	...
	if _, err := Digest(&pb.SpendAuthorizationPayload{WholesaleAccountId:"test-account",SettlementDomainId: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Domain: "wrong-domain"}); err == nil {
		t.Fatal("wrong domain accepted")
	}
	if err := Verify(nil); err == nil {
		t.Fatal("nil authorization accepted")
	}
	if err := Verify(&pb.SpendAuthorization{Payload: &pb.SpendAuthorizationPayload{WholesaleAccountId:"test-account",SettlementDomainId: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Domain: Domain, Payer: []byte{1}}}); err == nil {
		t.Fatal("short payer accepted")
	}
	payload := &pb.SpendAuthorizationPayload{WholesaleAccountId:"test-account",SettlementDomainId: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", Domain: Domain, Payer: make([]byte, 20)}
