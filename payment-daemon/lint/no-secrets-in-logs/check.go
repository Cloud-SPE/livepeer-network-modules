...
// "private_key". Pairs with plan 0017 §5.5 + §11.6 ("password scrubbing
// — simple []byte zeroing post-decrypt"): once the warm key is loaded,
// no log call may reference it (or the password) — the lint keeps that
// promise mechanical.
//
...
// not flagged in v1; operators either refactor them to literals or add
// a `//nolint:nosecrets` justification.
//
// Source: ported from
// `livepeer-modules-project/payment-daemon/lint/no-secrets-in-logs/check.go`
// at tag v4.1.3 (SHA caddeb342edb88faeea6a52e83a24c55704f0ef5). The
// remediation pointer was redirected from plan 0013 (the prior repo's
// structured-logging plan) to plan 0017's password-scrubbing locks
// (§5.5 + §11.6). Per AGENTS.md lines 62-66 the port is a deliberate
// carryover; the source path and tag are recorded in the introducing
// commit.
package main
...
func DefaultSkip(path string) bool {
	...
	// The lint program itself defines the deny-list strings as literals;
	// it's not a logging call but the analyzer can still false-flag it.
	// Skip the lint package proactively — it is its own ground truth.
	if strings.Contains(path, "lint/no-secrets-in-logs") {
		return true
	}
	return false
}
...
// String renders the finding in the lint's canonical format.
func (f Finding) String() string {
	return fmt.Sprintf(
		"%s:%d:%d: %s: %s\n  Remediation: remove the attribute, or use a derived value (e.g., a fingerprint hash) instead of the secret itself.\n  See: docs/exec-plans/active/0017-warm-key-handling-design.md §5.5",
