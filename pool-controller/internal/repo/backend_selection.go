...
func (r *StateRepo) SyncBackendSelectionStates(cfg *config.Config) error {
	if r == nil || r.db == nil {
		return fmt.Errorf("repo is not open")
	}
	if cfg == nil {
		...
		for _, member := range cfg.Members {
			for _, backend := range member.Backends {
				for _, offering := range backend.Offerings {
					key := backendSelectionStateKey(member.EthAddress, backend.ID, offering.CapabilityID, offering.OfferingID)
					if raw := b.Get([]byte(key)); raw != nil {
						...
						if existing.MemberEthAddress == "" {
							existing.MemberEthAddress = member.EthAddress
							changed = true
						}
						if existing.BackendID == "" {
							existing.BackendID = backend.ID
							changed = true
						}
						if existing.CapabilityID == "" {
							existing.CapabilityID = offering.CapabilityID
							changed = true
						}
						if existing.OfferingID == "" {
							existing.OfferingID = offering.OfferingID
							changed = true
						}
						if existing.State == "" {
					...
					state := defaultBackendSelectionState(member.EthAddress, backend, offering, now)
