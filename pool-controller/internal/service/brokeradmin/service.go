...
func (c *Client) postReload() error {
	...
	if err != nil {
		return fmt.Errorf("broker reload request failed: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return fmt.Errorf("broker reload request returned status %d", resp.StatusCode)
	}
	return nil
}
