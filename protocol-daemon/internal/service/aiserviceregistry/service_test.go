...
func TestSetServiceURISubmitsTxIntent(t *testing.T) {
	...
	id, err := svc.SetServiceURI(context.Background(), " https://ai.example.com ")
	if err != nil {
		t.Fatal(err)
	}
	if reg.uri != "https://ai.example.com" {
		t.Fatalf("normalized uri = %q", reg.uri)
	}
	if got, want := sub.params.Kind, "SetAIServiceURI"; got != want {
	...
	if sub.params.Metadata["ai_service_uri"] != "https://ai.example.com" {
...
func TestSetServiceURIPropagatesPackError(t *testing.T) {
	...
	if _, err := svc.SetServiceURI(context.Background(), "https://ai.example.com"); err == nil {
