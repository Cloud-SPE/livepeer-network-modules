...
func TestSubstringMatch(t *testing.T) {
	// Key has "password" as a substring.
	src := `package p
import "log/slog"
...
func TestSlogLogWithContextAndLevel(t *testing.T) {
	// slog.Log(ctx, level, msg, key, value, ...) — key starts at index 3.
	src := `package p
import (
	"context"
	"log/slog"
...
func TestNonLiteralKeysIgnored(t *testing.T) {
	// A runtime-computed key isn't flagged — we'd need type resolution
	// to know what's in there. Operators should refactor to literals.
	src := `package p
import (
	"fmt"
	"log/slog"
...
func TestNonSlogMethodNotFlagged(t *testing.T) {
	// A method called "Info" on something unrelated (heuristic limitation
	// — we don't do type resolution) — see classifySlogCall's heuristic
	// note. This test documents the current behavior as "yes it flags";
	// if it turns out to be noisy in practice, add type-resolution.
	src := `package p
type logger struct{}
...
func f() {
	...
	out := findingsFor(t, src)
	if len(out) != 1 {
		t.Errorf("heuristic: any method named Info is treated as slog-like; got %d", len(out))
	}
}
...
func TestFindingString(t *testing.T) {
	...
	s := f.String()
	for _, want := range []string{"a.go:5:2:", "nosecrets:", "Remediation:", "docs/exec-plans/completed/0013"} {
...
func TestCheckDirSkipsTestdataAndVendor(t *testing.T) {
	...
	// in main.go. Verify the plumbing.
	findings, err := CheckDir("./testdata", func(_ string) bool { return false })
	if err != nil {
		t.Fatalf("CheckDir: %v", err)
	}
	// testdata/good.go has no issues; testdata/bad.go has one.
...
func TestRunCleanExitsZero(t *testing.T) {
	// The main repo's live code has zero findings at time of writing.
	// Use a tmpdir with one benign file.
	tmp := t.TempDir()
	path := tmp + "/a.go"
