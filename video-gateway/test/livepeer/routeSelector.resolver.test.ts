...
test("video route selector returns no candidates when resolver has pruned unhealthy offerings", async (t) => {
  const resolverProtoRoot = await locateResolverProtoRoot();
  if (!resolverProtoRoot) {
    t.diagnostic("skipping: proto root not found");
    return;
  }
  const tmpDir = await fs.mkdtemp(path.join(tmpdir(), "video-route-selector-"));
