...
docker build --target runtime-nvidia \
  ...
  -t tztcloud/abr-runner:v1.2.0 \
  -f abr-runner/Dockerfile ..

docker build --target runtime-intel \
  --build-arg CODECS_IMAGE=tztcloud/codecs-builder:v1.2.0 \
  -t tztcloud/abr-runner-intel:v1.2.0 \
  -f abr-runner/Dockerfile ..

docker build --target runtime-amd \
  --build-arg CODECS_IMAGE=tztcloud/codecs-builder:v1.2.0 \
  -t tztcloud/abr-runner-amd:v1.2.0 \
  -f abr-runner/Dockerfile ..
```
...
| Image | Platforms |
|---|---|
| `abr-runner:v1.2.0` (nvidia) | linux/amd64 only |
| `abr-runner-intel:v1.2.0` | linux/amd64 only |
| `abr-runner-amd:v1.2.0` | linux/amd64 only |
