...
docker build --target runtime-nvidia \
  --build-arg CODECS_IMAGE=tztcloud/codecs-builder:v1.3.0 \
  -t tztcloud/transcode-runner:v1.3.0 \
  -f transcode-runner/Dockerfile ..

# Intel QSV (oneVPL) + VAAPI
docker build --target runtime-intel \
  --build-arg CODECS_IMAGE=tztcloud/codecs-builder:v1.3.0 \
  -t tztcloud/transcode-runner-intel:v1.3.0 \
  -f transcode-runner/Dockerfile ..

# AMD VAAPI
docker build --target runtime-amd \
  --build-arg CODECS_IMAGE=tztcloud/codecs-builder:v1.3.0 \
  -t tztcloud/transcode-runner-amd:v1.3.0 \
  -f transcode-runner/Dockerfile ..
```
...
| Image | Platforms |
|---|---|
| `transcode-runner:v1.3.0` (nvidia) | linux/amd64 only |
| `transcode-runner-intel:v1.3.0` | linux/amd64 only |
| `transcode-runner-amd:v1.3.0` | linux/amd64 only |
