...
gateway session, persist record, then revoke/complete on stop.
"""
...
class _FakeGateway:
    ...
    async def open_session(
        ...
        sid = f"gateway-sess-{self.session_counter}"
...
def _build_lifecycle() -> tuple[
    StreamLifecycle, StreamRepository, _FakeGateway, _FakeEgress, _FakeYouTube
]:
    ...
    lifecycle = StreamLifecycle(
        ...
        gateway_customer_bearer="sk-test-pipeline",
        gateway_public_url="http://b",
        public_base_url="http://p",
        default_rtmp_url="rtmp://nginx-rtmp:1935/live",
    )
    return lifecycle, repo, gateway, egress, youtube
...
async def test_create_orchestrates_register_then_open_then_persist() -> None:
    lifecycle, repo, gateway, egress, youtube = _build_lifecycle()
    ...
    # Order: youtube create → egress register → gateway open.
    assert youtube.broadcasts_created == 1
    assert egress.registered == [resp.stream_id]
    assert len(gateway.calls) == 1

    # Gateway gets customer's persona/avatar/voice/llm/render, plus
    # the egress URL+auth from the egress register response.
    body = gateway.calls[0]["params"]
    ...
    assert rec.gateway_session_id == "gateway-sess-1"
    assert rec.gateway_session_child_bearer == "vtbs_gateway-sess-1"
...
async def test_stop_revokes_egress_completes_youtube_marks_ended() -> None:
    lifecycle, _repo, _gateway, egress, youtube = _build_lifecycle()
    resp = await lifecycle.create(_create_request())
...
async def test_create_propagates_gateway_failure() -> None:
    lifecycle, repo, gateway, *_ = _build_lifecycle()
    gateway.fail = True
...
async def test_create_request_rejects_extra_fields() -> None:
    """Schema-level pinning: customers can't smuggle gateway fields
